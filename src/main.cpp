...
const string strMessageMagic = "greenblocks Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if (pindexBest->nHeight == 0 ) // premine 100,000,000
    {
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    nRewardCoinYear = COIN_YEAR_REWARD; // 500%
    int64_t nSubsidy = 1 * COIN;

 
    if (pindexBest->nHeight <= 1000)
    {
        nSubsidy = nCoinAge / 100 * nRewardCoinYear / 365; // 5%
    }
    else if (pindexBest->nHeight <= 2000)
    {
        nSubsidy = nCoinAge / 50 * nRewardCoinYear / 365; // 10%
    }
    else if (pindexBest->nHeight <= 5000)
    {
        nSubsidy = nCoinAge * nRewardCoinYear / 365 * 20; // 10k%
    }
    else if (pindexBest->nHeight <= 10000)
    {
        nSubsidy = nCoinAge * nRewardCoinYear / 365; // 500%
    }
    else if (pindexBest->nHeight <= 20000)
    {
        nSubsidy = nCoinAge / 5 * nRewardCoinYear / 365; // 100%
    }
    ...
    {
        nSubsidy = nCoinAge / 2 * nRewardCoinYear / 365; // 250%
    }
    else if (pindexBest->nHeight <= 90000)
    {
        nSubsidy = nCoinAge / 4 * nRewardCoinYear / 365; // 125%
    }
    else if (pindexBest->nHeight <= 110000)
    {
        nSubsidy = nCoinAge / 8 * nRewardCoinYear / 365; // 62.5%
    }
    else if (pindexBest->nHeight <= 130000)
    {
        nSubsidy = nCoinAge / 16 * nRewardCoinYear / 365; // 31.25%
    }
    else if (pindexBest->nHeight <= 150000)
    {
        nSubsidy = nCoinAge / 32 * nRewardCoinYear / 365; // ~15%
    }
    else if (pindexBest->nHeight > 150000)
    {
        nSubsidy = nCoinAge / 64 * nRewardCoinYear / 365; // ~7.8%
    }
   
    if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfStakeReward(): create=%s nCoinAge=%"PRId64"\n", FormatMoney(nSubsidy).c_str(), nCoinAge);

    return max(nSubsidy, 1 * COIN) + nFees; // block reward will never be lower than 1 Coin
}
...
// greenblocks: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "greenblocks", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
unsigned char pchMessageStart[4] = { 0x2b, 0x4d, 0x3c, 0x2b };
